...
(currently `1`), and `herdr control` bridges one to stdin and stdout so an SSH
exec channel can carry it without a PTY.

Open the stream with `control.open` as the first request on a fresh connection.
The response reports the server `boot_id`, `connection_id`, version, protocol,
and capabilities. Every later line on the connection is a request; responses,
pushed subscription events, and terminal records share the socket in one order.
`events.subscribe` adds subscriptions to the same connection instead of taking
it over, and `ping` answers locally. Blocking helpers (`events.wait`,
`agent.wait`, `agent.prompt`, `pane.wait_for_output`) and `pane.graphics.stream`
are refused with `unsupported_in_control_stream`; use a separate connection for
them. `control.close`, or closing the connection, releases everything the stream
owned.

```json
{"id":"c","method":"control.open","params":{}}
{"id":"a","method":"terminal.attach","params":{"target":"w1:p1","answer_queries":"client","history_limit_bytes":1048576}}
{"id":"g","method":"tab.set_geometry","params":{"tab_id":"w1:t1","cols":120,"rows":40,"cell_width_px":8,"cell_height_px":16}}
...
`terminal.attach` takes a pane id, terminal id, or agent name and makes the
stream the writable owner of that terminal, like a direct attach; pass
`takeover: true` to evict another owner. The response carries the `attach_id`,
...
focus reports the client's emulator produced. `terminal.detached` reports
`takeover` or `closed`. The record shapes are in the bundled schema under
`schemas.control_record`.

`answer_queries` decides which emulator answers device attributes, XTGETTCAP,
and OSC color queries. With `client` (the default) Herdr suppresses its own
replies while the attach owns the terminal, so exactly one emulator answers;
with `server` Herdr answers as usual.

Sizes follow the tab layout by default: claim a tab with `tab.set_geometry` and
Herdr lays the tab out in the claimed area. Pass `chrome: "none"` to lay the
tab out without Herdr's borders, gaps, and scrollbar gutters, so the pane
rectangles tile the area exactly and the client draws its own dividers; the
default `server` keeps the configured chrome. Every geometry change while a
control stream is open, whichever client caused it, sends a `tab.layout` record
on the stream before the terminals are resized. It carries the tab's
`PaneLayoutSnapshot` with each pane's rectangle set to the inner area its
terminal is sized to, after borders, gutters, and zoom. Because the record
shares the ordered lane with `terminal.output`, a client always has the new
rectangles before any redraw drawn at the new size. `session.snapshot` layouts
use the server's shared view area and outer pane rectangles, so treat them as
topology until the first `tab.layout` for a tab arrives. The claim follows the
same rules as a Herdr client viewing the tab, so the last client to interact
wins. An attach with `geometry: "terminal"` instead sizes one terminal with
`terminal.resize` and locks the tab layout out of it; when it detaches, the
tab's geometry controller sizes the terminal again.
