...
`control` opens a socket API control stream and bridges it to stdin and stdout
as newline-delimited JSON. The first line printed is the `control.open`
response with the server's boot id and capabilities; requests written to stdin
get their responses, subscription events, and raw `terminal.*` records on
stdout. It exists for clients on an SSH exec channel that render panes with
their own terminal emulator; the socket API documentation covers the
`terminal.attach` family. EOF on stdin closes the stream.
...
| `HERDR_CONTROL_CLIENT` | `name/version` that `herdr control` declares on `control.open`. |
| `HERDR_CONTROL_PROTOCOL` | Highest control stream protocol `herdr control` negotiates. Unset means protocol 1; set `2` to opt in. With neither variable set no client identity is sent at all. |
