...
`control` 打开一条套接字 API 控制流,并以换行分隔的 JSON 桥接到 stdin 和 stdout。打印的第一行是带服务器 boot id 和 capabilities 的 `control.open` 响应;写入 stdin 的请求会在 stdout 上得到响应、订阅事件和原始 `terminal.*` 记录。它面向在 SSH exec 通道上用自己的终端模拟器渲染窗格的客户端,`terminal.attach` 系列方法见套接字 API 文档。stdin 的 EOF 会关闭控制流。
