...
コントロールストリームは、クライアントがターミナルエミュレータのように Herdr を操作するために開き続ける 1 本の接続です。通常のリクエスト、イベント購読、そしてクライアント自身が描画するペインの生のターミナル出力を多重化します。対応するサーバーは `ping` の capabilities で `terminal_control_stream`（現在は `1`）を公開し、`herdr control` はストリームを stdin と stdout に橋渡しするので、PTY なしの SSH exec チャネルでも運べます。

新しい接続の最初のリクエストとして `control.open` を送ってストリームを開きます。レスポンスにはサーバーの `boot_id`、`connection_id`、バージョン、プロトコル、capabilities が含まれます。以降の行はすべてリクエストで、レスポンス、プッシュされる購読イベント、ターミナルレコードは 1 つの順序でソケットを共有します。`events.subscribe` は接続を占有せず同じ接続に購読を追加し、`ping` はローカルで応答します。ブロックするヘルパー（`events.wait`、`agent.wait`、`agent.prompt`、`pane.wait_for_output`）と `pane.graphics.stream` は `unsupported_in_control_stream` で拒否されるため、別の接続を使ってください。`control.close` または接続の切断で、ストリームが所有していたものをすべて解放します。
...
`terminal.attach` はペイン ID、ターミナル ID、またはエージェント名を受け取り、直接アタッチと同様にストリームをそのターミナルの書き込み可能な所有者にします。別の所有者を追い出すには `takeover: true` を渡します。レスポンスには `attach_id` が含まれ、その最初のレコードは `terminal.snapshot` です。アクティブ画面の ANSI（折り返しを解除したスクロールバックを含む `primary`、または `alternate`）、ターミナル状態を復元するシーケンス（モード、スクロール領域、タブストップ、文字集合、アクティブなペン、そしてプッシュされた Kitty スタックを含むキーボードプロトコル）を含む `state_ansi`、アクティブなペンだけを含む `pen_ansi`、カーソル、そして同じ事実をフィールドとして持つ `state` オブジェクトで構成されます。印字の後に最終列に留まっているカーソルは `pending_wrap` と `pending_wrap_cell` を報告します。後者はその位置から再印字するスタイル付きのセルで、クライアントのターミナルがサーバーと同じように次の文字を折り返すためのものです。PTY の読み取りが途中で終わったエスケープシーケンスは、スナップショットの直後に最初の `terminal.output` レコードとして続くため、クライアントは完全な形で解析できます。スナップショットはペインの出力ロックの下で取得されるため、より大きい `seq` を持つ `terminal.output` レコードはすべてその後に生成されたものです。`history_limit_bytes` はプライマリ履歴を制限し（既定 1 MiB、最大 8 MiB）、`truncated` は切り詰めを報告します。代替画面がアクティブな間はその画面のみを取得します。

`terminal.output` は生の PTY バイトを base64 で、アタッチごとの `seq` とともに運びます。クライアントが遅れるとサーバーは出力を破棄して `terminal.gap` を送るので、`terminal.snapshot` をリクエストして再同期してください。現在の `seq` を刻印した新しい `terminal.snapshot` が発行されます。`terminal.input` は、ブラケットペーストのラッパーやクライアントのエミュレータが生成したマウス・フォーカスレポートを含め、バイトをそのまま PTY に書き込みます。`terminal.detached` は `takeover` または `closed` を報告します。レコードの形式はバンドルされたスキーマの `schemas.control_record` にあります。

`answer_queries` は、デバイス属性、XTGETTCAP、OSC カラークエリにどちらのエミュレータが応答するかを決めます。`client`（既定）では、アタッチがターミナルを所有している間 Herdr は自身の応答を抑制し、ちょうど 1 つのエミュレータだけが応答します。`server` では Herdr が通常どおり応答します。

サイズは既定でタブレイアウトに従います。`tab.set_geometry` でタブを要求すると、Herdr は要求した領域でタブをレイアウトします。`chrome: "none"` を渡すと、Herdr の境界線、間隔、スクロールバーのガターなしでタブをレイアウトするため、ペインの矩形が領域をぴったり敷き詰め、クライアントが自前の区切り線を描けます。既定の `server` は設定済みのクロームを保ちます。コントロールストリームが開いている間は、どのクライアントが引き起こしたジオメトリ変更でも、ターミナルのサイズ変更前に `tab.layout` レコードがストリームに送られます。このレコードはタブの `PaneLayoutSnapshot` を運び、各ペインの矩形には、境界線、ガター、ズームを適用した後にターミナルが実際にサイズ設定される内側の領域が入ります。レコードは `terminal.output` と同じ順序付きレーンを共有するため、クライアントは新しいサイズで描画された再描画を受け取る前に、必ず新しい矩形を持っています。`session.snapshot` のレイアウトはサーバーの共有ビュー領域と外側のペイン矩形を使うため、タブの最初の `tab.layout` が届くまではトポロジーとして扱ってください。この要求はタブを表示している Herdr クライアントと同じ規則に従うため、最後に操作したクライアントが優先されます。`geometry: "terminal"` のアタッチは代わりに `terminal.resize` で 1 つのターミナルのサイズを決め、タブレイアウトをそのターミナルから締め出します。デタッチすると、タブのジオメトリコントローラーが再びそのターミナルのサイズを決めます。
